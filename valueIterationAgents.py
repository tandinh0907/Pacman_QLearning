    ...
    def runValueIteration(self):
        ...
        for number in range(self.iterations):
            ...
            for state in stateList:
                ...
                for act in action:
                    nextState = self.mdp.getTransitionStatesAndProbs(state, act)
                    actV = 0
                    for newState, prob in nextState:
                        actV += prob * (self.mdp.getReward(state, act, newState) + self.discount * self.values[newState])
                    v = max(v, actV)
                if v == -1000:
                    newValue[state] = 0
                else: 
